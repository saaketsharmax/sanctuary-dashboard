...
import { useAuthStore, mockPartner } from '@/lib/stores/auth-store'
import {
  LayoutDashboard,
  Briefcase,
  BarChart3,
        ...
        <div className="p-4 border-t">
          <div className="flex items-center gap-3 mb-3">
            <Avatar className="h-9 w-9">
              <AvatarFallback>{mockPartner.name.charAt(0)}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{mockPartner.name}</p>
              <p className="text-xs text-muted-foreground truncate">{mockPartner.email}</p>
            </div>
          </div>
