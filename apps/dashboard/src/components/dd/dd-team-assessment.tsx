...
const sentimentConfig = {
  positive: { color: 'bg-green-100 text-green-600', icon: CheckCircle2 },
  neutral: { color: 'bg-muted text-foreground', icon: MessageSquare },
  concerning: { color: 'bg-red-100 text-red-600', icon: AlertTriangle },
}
                ...
                <div className="flex flex-wrap gap-1">
                  {profile.previousStartups.map((ps, i) => (
                    <Badge key={i} variant="outline" className="text-[10px]">
                      {ps.name}: {ps.outcome}
                      {ps.verified && <CheckCircle2 className="h-3 w-3 ml-1 text-green-600" />}
                    </Badge>
                  ))}
                </div>
              </div>
            ...
            {profile.strengths.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {profile.strengths.map((s, i) => (
                  <Badge key={i} variant="secondary" className="text-[10px] bg-green-50 text-green-600">
                    {s}
                  </Badge>
                ))}
              </div>
